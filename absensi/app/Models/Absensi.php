...
class Absensi extends BaseModel {
    ...
    public function getDateRangeReport(int $pembimbingId, string $startDate, string $endDate): array {
        $st = $this->db->prepare("
            SELECT a.*, s.nama as siswa, s.kelas, s.nis, s.pembimbing_id
            FROM absensi a 
            JOIN siswa s ON a.siswa_id = s.id 
            ...
            ORDER BY a.waktu ASC
        ");
        $st->execute([$pembimbingId, $startDate, $endDate]);
        return $st->fetchAll();
    }
    ...
    public function countWithFilters(array $filters): int {
        ...
        if ($date !== '') {
            $sql .= " AND DATE(a.waktu) = ?";
            $params[] = $date;
        }
        
        $status = trim($filters['status'] ?? '');
        if ($status === 'dalam') {
            // Catatan: menggunakan ambang 150m sesuai tampilan admin
            $sql .= " AND a.jarak_m <= 150";
        } else if ($status === 'luar') {
            $sql .= " AND a.jarak_m > 150";
        }
