...
class AdminController extends Controller {
    ...
    public function absensi(): void {
        ...
        // Ambil parameter filter untuk pencarian
        ...
        $filters = [
            ...
            'date' => trim($_GET['date'] ?? ''),
            'status' => trim($_GET['status'] ?? ''), // 'dalam' | 'luar' | ''
        ];
        
        $absensi = new Absensi($db);
        // Hitung total sesuai filter
        $totalAbsensi = $absensi->countWithFilters($filters);
        $totalPages = max(1, (int)ceil($totalAbsensi / $limit));
        
        // Ambil data absensi sesuai filter + pagination
        $listAbsensi = $absensi->getWithFilters($filters, $offset, $limit);
    ...
    public function siswa(): void {
        ...
        $tempatId = (int)($_GET['tempat_pkl_id'] ?? 0);
        $searchStatus = trim($_GET['search_status'] ?? '');
        
        // Jika ada parameter pencarian atau filter, gunakan method pencarian
        if (!empty($searchNIS) || !empty($searchNama) || !empty($searchKelas) || 
            !empty($searchPembimbing) || !empty($searchTempat) || !empty($searchStatus) || $pembimbingId > 0 || $tempatId > 0) {
        ...
        $pembimbing = (new Pembimbing($db))->listAll();
        ...
        $this->view('admin/siswa', [
            ...
            'filters' => [
                ...
                'search_kelas' => $searchKelas,
                'search_pembimbing' => $searchPembimbing,
                'search_tempat' => $searchTempat,
                'pembimbing_id' => $pembimbingId,
                'tempat_pkl_id' => $tempatId
            ]
        ]);
    ...
    public function importProcess(): void {
        ...
        // Tidak perlu pembimbing_id dan tempat_pkl_id lagi karena sudah ada di CSV
        
        $siswa = new Siswa($db);
        $user = new User($db);
        $tempatPKL = new TempatPKL($db);
        $pembimbingModel = new Pembimbing($db);
        ...
        while (($data = fgetcsv($handle)) !== false) {
            if (count($data) < 7) {
                $results[] = [
                    'row' => $row,
                    'status' => 'error',
                    'message' => 'Data tidak lengkap (harus 7 kolom)'
                ];
                $errors++;
                $row++;
                continue;
            ...
            $nama = trim($data[3] ?? '');
            $kelas = trim($data[4] ?? '');
            $namaPembimbing = trim($data[5] ?? '');
            $namaTempat = trim($data[6] ?? '');
            ...
            try {
                ...
                if(!$pb){ 
                    ...
                    $pembimbingId = $pembimbingModel->create([
                        'nama' => $namaPembimbing,
                        'nip' => $uniqueNIP
                    ]);
                } else {
                ...
                if(!$tp){ 
                    ...
                    $tempatId = $tempatPKL->create([
                        'nama' => $namaTempat
                        // lat, lng, radius_m akan di-set default oleh model
                    ]);
                } else {
                    $tempatId = (int)$tp['id'];
                }
    ...
    public function tempatImport(): void {
        Auth::requireRole('admin');
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (!isset($_FILES['csv_file']) || $_FILES['csv_file']['error'] !== UPLOAD_ERR_OK) {
                $this->view('admin/tempat_import', [
                    'error' => 'File CSV tidak ditemukan atau error'
                ]);
                return;
            }
            ...
            try {
                ...
                if (!$header || count($header) < 4) {
                    throw new Exception('Format CSV tidak valid. Minimal harus ada 4 kolom: Nama Tempat, Pemilik, Alamat, Koordinat');
                }
                ...
                while (($data = fgetcsv($handle)) !== false) {
                    ...
                    if (count($data) < 4) {
                        $results[] = [
                            'row' => $row,
                            'status' => 'error',
                            'message' => 'Data tidak lengkap (minimal 4 kolom)'
                        ];
                        $errors++;
                        continue;
                    }
                    ...
                    if (empty($nama)) {
                        $results[] = [
                            'row' => $row,
                            'status' => 'error',
                            'message' => 'Nama tempat tidak boleh kosong'
                        ];
                        $errors++;
                        continue;
                    }
                    ...
                    $koordinatParts = preg_split('/[,;]/', $koordinat);
                    if (count($koordinatParts) !== 2) {
                        $results[] = [
                            'row' => $row,
                            'status' => 'error',
                            'message' => 'Format koordinat tidak valid. Gunakan format: latitude,longitude'
                        ];
                        $errors++;
                        continue;
                    }
                    
                    $lat = (float)trim($koordinatParts[0]);
                    $lng = (float)trim($koordinatParts[1]);
                ...
                // Log audit
                (new AuditLog($db))->add($_SESSION['user']['id'], 'import_tempat_pkl', "Total: " . count($results) . ", Success: $success, Errors: $errors");
                
                $this->view('admin/tempat_import_result', [
                    'results' => $results,
                    'total_rows' => count($results),
                    'success_count' => $success,
                    'error_count' => $errors
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            try {
                ...
                if (!empty($dataPembimbing['user_id'])) {
                    ...
                    if (empty($password)) {
                        throw new Exception('Password wajib diisi untuk membuat user account baru');
                    }
                    ...
                    // Update pembimbing dengan user_id
                    $pembimbingSuccess = $pembimbing->update($id, [
                        'nama' => $nama,
                        'nip' => $nip,
                        'user_id' => $userId
                    ...
                    if (!$pembimbingSuccess) {
                        throw new Exception('Gagal mengupdate user_id pembimbing');
                    }
                }
    ...
    public function report(): void {
        ...
        // Ambil daftar tempat PKL untuk dropdown
        $tempatPKL = (new TempatPKL($db))->listAll();
