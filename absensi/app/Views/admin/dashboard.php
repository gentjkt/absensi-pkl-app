...
<div class="dashboard-grid">
    ...
    <div class="card">
        ...
        <div class="realtime-content">
            ...
            <?php else: ?>
                <div class="absensi-list compact" id="absensiList">
                    <?php foreach($absensiTerbaru as $abs): ?>
                        <div class="absensi-item compact" data-id="<?= $abs['id'] ?>">
                            <div class="absensi-main">
                                <div class="siswa-compact">
                                    <strong class="nama-siswa"><?= htmlspecialchars($abs['siswa']) ?></strong>
                                    <span class="info-compact">
                                        <?= htmlspecialchars($abs['nis']) ?> | <?= htmlspecialchars($abs['kelas']) ?>
                                    </span>
                                </div>
                                <div class="waktu-compact">
                                    <span class="time-compact"><?= date('H:i', strtotime($abs['waktu'])) ?></span>
                                    <span class="date-compact"><?= date('d/m', strtotime($abs['waktu'])) ?></span>
                                </div>
                            </div>
                            
                            <div class="details-compact">
                                <span class="tempat-compact">🏢 <?= htmlspecialchars($abs['tempat_pkl'] ?? 'N/A') ?></span>
                                <span class="jarak-compact">📏 <?= number_format($abs['jarak_m'], 0) ?>m</span>
                                <?php 
                                    $jarak = (float)($abs['jarak_m'] ?? 0);
                                    if ($jarak <= 150) {
                                        echo '<span class="status-compact success">✅</span>';
                                    } else {
                                        echo '<span class="status-compact warning">⚠️</span>';
                                    }
                                ?>
                                <?php if(!empty($abs['selfie_path'])): ?>
                                    <a href="<?= htmlspecialchars($abs['selfie_path']) ?>" 
                                       target="_blank" class="selfie-compact" title="Lihat Selfie">
                                        📸
                                    </a>
                                <?php endif; ?>
                            </div>
...
/* Tampilan Compact untuk Absensi Realtime */
.absensi-list.compact {
    max-height: 400px;
}

.absensi-item.compact {
    padding: 10px;
    margin-bottom: 6px;
    border-radius: 6px;
    background: #ffffff;
